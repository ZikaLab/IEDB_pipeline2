...
def render_temp_perl(perl_src: Path, align_csv: Path, output_root: Path) -> Path:
    ...
    # Update Python script path to use BASE_DIR (Class II path)
    python_script_path = BASE_DIR / "ng_tc2-0.2.2-beta/src/tcell_mhcii.py"
    # Replace any old hardcoded paths (Class I or Class II) with dynamic path
    new_text = new_text.replace(
    ...
    # Also handle case where Perl script might already have ng_tc2 path
    new_text = new_text.replace(
